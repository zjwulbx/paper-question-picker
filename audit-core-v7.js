(function attachAuditCoreV7(root, factory) {
  "use strict";

  let base = root && root.AuditCore;
  if (typeof module === "object" && module.exports && typeof require === "function") {
    base = require("./audit-core-v6.js");
  }
  const api = factory(base);
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.AuditCore = api;
})(typeof globalThis !== "undefined" ? globalThis : this, function createAuditCoreV7(Base) {
  "use strict";

  if (!Base || typeof Base.sha256Hex !== "function") {
    throw new Error("v6 审计核心没有正确载入。");
  }

  const PROTOCOL_ID = "paper-question-picker/v7";
  const NORMALIZATION_ID = "nfc-course-table/v2";
  const RNG_ID = "sha256-ctr-split-hex-u64be-u32be-reject/v3";
  const SCHEDULE_ID = "variable-week-cyclic-weekly-disjoint-roles/v1";
  ...
  const D_INPUT = "paper-question-picker/input/v7\0";
  const D_COMMIT = "paper-question-picker/commitment/v7\0";
  const D_QUESTION_RNG_KEY = "paper-question-picker/rng-key/questions/v7\0";
  const D_QUESTION_RNG_BLOCK = "paper-question-picker/rng-block/questions/v7\0";
  const D_PRESENTER_RNG_KEY = "paper-question-picker/rng-key/presenters/v7\0";
  const D_PRESENTER_RNG_BLOCK = "paper-question-picker/rng-block/presenters/v7\0";
  const D_PLAN = "paper-question-picker/plan/v7\0";
  ...
  function canonicalizeInput(studentNames, courseWeeks) {
    ...
    if (studentNames.length < 12 || studentNames.length > MAX_ITEMS) {
      fail("INVALID_INPUT", "学生人数必须为 12 至 10000 人。");
    }
    ...
    if (largestWeek * 4 > students.length) {
      fail("INVALID_INPUT", "学生人数不足以保证同一周的报告人与提问人全部互不重复。");
    }
  ...
  function createPrng(seedHex, inputDigestHex, purpose) {
    ...
    if (purpose === undefined || purpose === "questions") {
      ...
      fail("INVALID_RANDOM_PURPOSE", "v7 随机流用途必须是 questions 或 presenters。");
    }
  ...
  function makePlan(input, seedHex) {
    ...
    const questionOffsets = questionRandom.shuffle([largestWeek, largestWeek * 2, largestWeek * 3]);
    const weeks = [];
    let paperCursor = 0;
    ...
    input.courseWeeks.forEach(function buildWeek(courseWeek, weekIndex) {
      const assignments = courseWeek.papers.map(function buildAssignment(_paper, paperOffset) {
        ...
        return {
          ...
          studentIndexes: questionOffsets.map(function questionerIndex(offset) {
            return roleOrder[(paperIndex + offset) % count];
          }),
    ...
    plan.weeks.forEach(function inspectWeek(week, weekIndex) {
      ...
      week.assignments.forEach(function inspectAssignment(assignment, paperOffset) {
        ...
        if (!Number.isInteger(assignment.presenterIndex) || assignment.presenterIndex < 0 || assignment.presenterIndex >= count) {
          fail("INVALID_PLAN", "v7 报告人下标超出范围。");
        }
        if (assignment.studentIndexes.includes(assignment.presenterIndex)) {
          fail("INVALID_PLAN", "v7 同一篇论文的报告人不得与提问人重复。");
        }
      ...
      if (weeklyQuestioners.length !== weekSize * 3 || new Set(weeklyQuestioners).size !== weekSize * 3) {
        fail("INVALID_PLAN", "v7 同一周的提问学生必须互不重复。");
      }
      const weeklyQuestionerSet = new Set(weeklyQuestioners);
      if (new Set(weeklyPresenters).size !== weekSize ||
          weeklyPresenters.some(function overlapsQuestioner(index) { return weeklyQuestionerSet.has(index); })) {
        fail("INVALID_PLAN", "v7 同一周的报告人与提问人必须全部互不重复。");
      }
      paperCursor += weekSize;
    });
    if (paperCursor !== count || questionTotals.some(function notThree(total) { return total !== 3; })) {
      fail("INVALID_PLAN", "v7 论文覆盖或学生提问总次数不正确。");
    }
    if (presenters.length !== count || new Set(presenters).size !== count) {
      fail("INVALID_PLAN", "v7 每位学生必须恰好报告一篇论文。");
    }
    return clone(plan);
  }
      ...
      weeks: schedule.weeks.map(function planWeek(week, weekIndex) {
        ...
        const entry = {
          ...
          assignments: week.assignments.map(function planAssignment(assignment, offset) {
            if (assignment.paper !== courseWeek.papers[offset]) fail("INVALID_SCHEDULE", "v7 论文顺序与规范输入不一致。");
            if (!indexById.has(assignment.presenterId)) fail("INVALID_SCHEDULE", "v7 安排含有未知报告人编号。");
            return {
              paperIndex: paperCursor + offset,
              studentIndexes: assignment.studentIds.map(function questioner(id) {
                if (!indexById.has(id)) fail("INVALID_SCHEDULE", "v7 安排含有未知提问学生编号。");
  ...
  function inspectSchedule(schedule, withAudit) {
    exactKeys(schedule, withAudit ? ["students", "weeks", "createdAt", "audit"] : ["students", "weeks", "createdAt"],
      "INVALID_SCHEDULE", "v7 抽签安排");
    requireIso(schedule.createdAt, "INVALID_SCHEDULE", "v7 抽签创建时间");
    if (!Array.isArray(schedule.students) || !Array.isArray(schedule.weeks)) fail("INVALID_SCHEDULE", "v7 抽签安排缺少学生或周次。");
    schedule.students.forEach(function student(student, index) {
      exactKeys(student, ["id", "name"], "INVALID_SCHEDULE", "v7 学生记录");
      if (student.id !== "student-" + (index + 1) || typeof student.name !== "string") {
        fail("INVALID_SCHEDULE", "v7 学生编号或姓名不正确。");
      }
    });
    let complete = true;
    schedule.weeks.forEach(function week(week, index) {
      exactKeys(week, ["id", "label", "assignments", "revealed"], "INVALID_SCHEDULE", "v7 周次记录");
      if (week.id !== "week-" + (index + 1) || typeof week.label !== "string" || !Array.isArray(week.assignments) ||
        ...
        fail("INVALID_SCHEDULE", "v7 周次编号、标题、论文数量或揭晓状态不正确。");
      }
      if (!week.revealed.every(Boolean)) complete = false;
      week.assignments.forEach(function assignment(assignment) {
        exactKeys(assignment, ["paper", "studentIds", "presenterId"], "INVALID_SCHEDULE", "v7 论文安排");
        if (typeof assignment.paper !== "string" || !Array.isArray(assignment.studentIds) ||
          ...
          fail("INVALID_SCHEDULE", "v7 论文安排格式不正确。");
        }
      });
    });
    ...
    if (!arraysEqual(raw.students, input.students) || !courseWeeksEqual(raw.courseWeeks, input.courseWeeks)) {
      fail("INVALID_SCHEDULE", "v7 安排中的输入不是规范形。");
    }
  ...
  function inspectAudit(schedule) {
    ...
    exactKeys(schedule.audit, ["protocolId", "normalizationId", "rngId", "scheduleId", "issuedAt", "inputDigest",
      "commitment", "planDigest", "seedHex", "commitmentConfirmedAt"], "INVALID_AUDIT", "v7 私密审计元数据");
    if (schedule.audit.protocolId !== PROTOCOL_ID || schedule.audit.normalizationId !== NORMALIZATION_ID ||
      ...
      fail("UNSUPPORTED_PROTOCOL", "v7 抽签审计协议标识不受支持。");
    }
    requireIso(schedule.audit.issuedAt, "INVALID_AUDIT", "v7 承诺签发时间");
    if (schedule.audit.issuedAt !== schedule.createdAt) fail("INVALID_AUDIT", "v7 承诺签发时间与安排创建时间不一致。");
    requireHex32(schedule.audit.inputDigest, "INVALID_AUDIT", "v7 输入摘要");
    requireHex32(schedule.audit.commitment, "INVALID_AUDIT", "v7 承诺码");
    requireHex32(schedule.audit.planDigest, "INVALID_AUDIT", "v7 计划摘要");
    requireHex32(schedule.audit.seedHex, "INVALID_AUDIT", "v7 随机种子");
    if (schedule.audit.commitmentConfirmedAt !== null) {
      requireIso(schedule.audit.commitmentConfirmedAt, "INVALID_AUDIT", "v7 承诺确认时间");
      if (Date.parse(schedule.audit.commitmentConfirmedAt) < Date.parse(schedule.createdAt)) {
        fail("INVALID_AUDIT", "v7 承诺确认时间不能早于安排创建时间。");
      }
    } else if (schedule.weeks.some(function revealed(week) { return week.revealed.some(Boolean); })) {
      fail("INVALID_AUDIT", "v7 尚未确认公开承诺的安排不能包含已揭晓结果。");
    }
    ...
    if (digest !== schedule.audit.inputDigest || commitment !== schedule.audit.commitment ||
      ...
      fail("AUDIT_MISMATCH", "v7 私密审计数据无法重现当前提问人与报告人安排。");
    }
    return inspected;
  }
  ...
  function createPublicReceiptV7(schedule) {
  ...
  function parsePublicReceiptV7(candidate) {
    exactKeys(candidate, RECEIPT_KEYS, "INVALID_RECEIPT", "v7 公开承诺凭证");
    if (candidate.format !== RECEIPT_FORMAT || candidate.version !== FORMAT_VERSION) {
      fail("INVALID_RECEIPT", "这不是受支持的 v7 论文提问抽签公开承诺凭证。");
    }
    if (candidate.protocolId !== PROTOCOL_ID || candidate.normalizationId !== NORMALIZATION_ID ||
      ...
      fail("UNSUPPORTED_PROTOCOL", "v7 公开承诺凭证协议标识不受支持。");
    }
    requireIso(candidate.issuedAt, "INVALID_RECEIPT", "v7 承诺签发时间");
    const counts = candidate.weekPaperCounts;
    if (!Number.isInteger(candidate.studentCount) || candidate.studentCount < 12 || candidate.studentCount > MAX_ITEMS ||
        ...
        Math.max.apply(null, counts) * 4 > candidate.studentCount || candidate.studentsPerPaper !== 3 ||
        candidate.finalSelectionsPerStudent !== 3 || candidate.presentersPerPaper !== 1 ||
        candidate.finalPresentationsPerStudent !== 1) {
      fail("INVALID_RECEIPT", "v7 公开承诺凭证的数量或分周结构不正确。");
    }
    requireHex32(candidate.inputDigest, "INVALID_RECEIPT", "v7 输入摘要");
    requireHex32(candidate.commitment, "INVALID_RECEIPT", "v7 承诺码");
    return clone(candidate);
  }

  function confirmCommitmentV7(schedule, at) {
    ...
    const confirmedAt = requireIso(at === undefined ? new Date().toISOString() : at, "INVALID_CONFIRMATION_TIME", "v7 承诺确认时间");
    if (Date.parse(confirmedAt) < Date.parse(schedule.createdAt)) fail("INVALID_CONFIRMATION_TIME", "v7 承诺确认时间不能早于安排创建时间。");
    if (schedule.audit.commitmentConfirmedAt && schedule.audit.commitmentConfirmedAt !== confirmedAt) {
      fail("COMMITMENT_ALREADY_CONFIRMED", "v7 承诺已经确认，不能改写确认时间。");
    }
    schedule.audit.commitmentConfirmedAt = confirmedAt;
    return schedule;
  }

  function createFinalReportV7(schedule, options) {
    const inspected = inspectAudit(schedule);
    ...
    const completedAt = options && options.completedAt !== undefined
      ? requireIso(options.completedAt, "INVALID_COMPLETION_TIME", "v7 报告生成时间")
      : new Date().toISOString();
    if (Date.parse(completedAt) < Date.parse(schedule.audit.commitmentConfirmedAt)) {
      fail("INVALID_COMPLETION_TIME", "v7 报告生成时间不能早于承诺确认时间。");
    }
    return {
      format: REPORT_FORMAT,
      version: FORMAT_VERSION,
      completedAt: completedAt,
      receipt: createPublicReceiptV7(schedule),
  ...
  function parseFinalReportV7(candidate) {
    exactKeys(candidate, REPORT_KEYS, "INVALID_REPORT", "v7 最终审计报告");
    if (candidate.format !== REPORT_FORMAT || candidate.version !== FORMAT_VERSION) {
      fail("INVALID_REPORT", "这不是受支持的 v7 论文提问抽签最终审计报告。");
    }
    requireIso(candidate.completedAt, "INVALID_REPORT", "v7 报告生成时间");
    const receipt = parsePublicReceiptV7(candidate.receipt);
    if (Date.parse(candidate.completedAt) < Date.parse(receipt.issuedAt)) fail("INVALID_REPORT", "v7 报告生成时间不能早于承诺签发时间。");
    requireHex32(candidate.seed, "INVALID_REPORT", "v7 报告随机种子");
    requireHex32(candidate.planDigest, "INVALID_REPORT", "v7 计划摘要");
    ...
    if (!arraysEqual(input.students, candidate.students) || !courseWeeksEqual(input.courseWeeks, candidate.courseWeeks)) {
      fail("INVALID_REPORT", "v7 报告输入不是规范形。");
    }
    ...
    if (receipt.studentCount !== input.students.length || receipt.paperCount !== input.students.length ||
      ...
      fail("INVALID_REPORT", "v7 报告输入数量或分周结构与内嵌承诺凭证不一致。");
    }
  ...
  function verifyReceiptAndReportV7(receiptCandidate, reportCandidate) {
    let receipt;
    let report;
    try {
      receipt = parsePublicReceiptV7(receiptCandidate);
      report = parseFinalReportV7(reportCandidate);
    } catch (error) {
      return verificationFailure(error && error.code ? error.code : "INVALID_AUDIT_FILE",
        error instanceof Error ? error.message : "v7 审计文件格式不正确。");
    }
    try {
      ...
      return verificationFailure(error && error.code ? error.code : "REPLAY_FAILED",
        error instanceof Error ? error.message : "v7 审计重放失败。");
    }
  }

  function isV7Schedule(schedule) {
    return Boolean(schedule && schedule.audit && schedule.audit.protocolId === PROTOCOL_ID);
  }

  function createPublicReceiptAny(schedule) {
    return isV7Schedule(schedule) ? createPublicReceiptV7(schedule) : Base.createPublicReceipt(schedule);
  }

  function confirmCommitmentAny(schedule, at) {
    return isV7Schedule(schedule) ? confirmCommitmentV7(schedule, at) : Base.confirmCommitment(schedule, at);
  }

  function createFinalReportAny(schedule, options) {
    return isV7Schedule(schedule) ? createFinalReportV7(schedule, options) : Base.createFinalReport(schedule, options);
  }

  function parsePublicReceiptAny(candidate) {
    if (candidate && candidate.version === FORMAT_VERSION) return parsePublicReceiptV7(candidate);
    if (candidate && (candidate.version === 4 || candidate.version === 5 || candidate.version === 6)) {
      return Base.parsePublicReceipt(candidate);
    }
    fail("UNSUPPORTED_VERSION", "公开承诺凭证版本不受支持。");
  }

  function parseFinalReportAny(candidate) {
    if (candidate && candidate.version === FORMAT_VERSION) return parseFinalReportV7(candidate);
    if (candidate && (candidate.version === 4 || candidate.version === 5 || candidate.version === 6)) {
      return Base.parseFinalReport(candidate);
    }
  ...
  function verifyReceiptAndReportAny(receipt, report) {
    ...
    if (receiptVersion === FORMAT_VERSION) return verifyReceiptAndReportV7(receipt, report);
    if (receiptVersion === 4 || receiptVersion === 5 || receiptVersion === 6) {
      return Base.verifyReceiptAndReport(receipt, report);
    }
